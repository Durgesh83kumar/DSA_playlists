...
def take_input_binary_tree():
    ...
    print(f"Enter the right child of the {data}: ")
...
def take_input_level_wise():
    ...
    while(len(queue) != 0):
        ...
        left_child_data = int(input(f"Enter left child data for {current_node.data}: "))
        if(left_child_data != -1):
            left_node = BinaryTreeNode(left_child_data)
            current_node.left = left_node
            queue.append(left_node)

        right_child_data = int(input(f"Enter right child data for {current_node.data}: "))
